...
-- ============================================================
--  (ทางเลือก) กันรายการประเมิน "ซ้ำทุกช่อง" ที่ระดับฐานข้อมูล
--   เทียบ: เวลา + ผู้ประเมิน + เจ้าหน้าที่ + คะแนน 5 ข้อ + ข้อเสนอแนะ
--   (ตัดช่องว่างหัวท้าย และถือว่าข้อเสนอแนะว่าง = ไม่มีข้อเสนอแนะ)
--  ⚠️ ต้องกดปุ่ม "ลบรายการประเมินที่ซ้ำ" ในเว็บให้ไม่มีรายการซ้ำก่อน
--     ไม่งั้นสร้าง index ไม่สำเร็จ (จะ error ว่ามีค่าซ้ำอยู่)
-- ============================================================
create unique index if not exists evaluations_dedup_uidx
  on public.evaluations (
    created_at, btrim(evaluator), btrim(staff),
    speed, problem_solving, communication, service_mind, satisfaction,
    coalesce(btrim(comment), '')
  );
