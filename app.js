...
const pctTxt = x => (num(x)/5*100).toFixed(1)+'%';
...
function criteriaRows(vals){return criteria.map(c=>{const v=num(vals[c.key]);return '<div class="criteria-row"><b>'+c.no+'</b><div><div>'+esc(c.shortTitle)+'</div><div class="bar" style="margin-top:7px"><div class="fill" style="width:'+pct(v)+'%;background:linear-gradient(90deg,'+c.color+',var(--mint))"></div></div></div><div style="text-align:center"><span class="tag '+tone(v)+'">'+fmt(v)+'</span><div class="mini" style="margin-top:3px">'+pctTxt(v)+'</div></div></div>';}).join('')||'<div class="empty">ไม่มีข้อมูล</div>';}
function rankRow(p,i,risk){return '<div class="rank-row"><div><div class="rank-name">'+i+'. '+esc(p.name)+'</div><div class="rank-meta">'+p.count+' รายการ | '+esc(p.band)+' | แนวโน้ม '+(p.trend>=0?'+':'')+p.trend.toFixed(2)+'</div><div class="bar" style="margin-top:8px"><div class="fill" style="width:'+pct(p.avg)+'%;background:'+(risk?'linear-gradient(90deg,var(--rose),var(--amber))':'linear-gradient(90deg,var(--cyan),var(--mint))')+'"></div></div></div><div class="score">'+fmt(p.avg)+'<div class="rank-meta" style="font-weight:700;margin-top:2px">'+pctTxt(p.avg)+'</div></div></div>';}
...
// ---------- สรุปรายเจ้าหน้าที่ ----------
function renderPeople(){const rows=data.people.filter(p=>!filter||p.name.toLowerCase().includes(filter.toLowerCase()));$('content').innerHTML='<div class="toolbar"><input class="input search" value="'+esc(filter)+'" oninput="filter=this.value;render()" placeholder="ค้นหาชื่อเจ้าหน้าที่"><span class="mini">'+data.people.length+' คน</span></div><div class="heat"><div class="head">เจ้าหน้าที่</div>'+criteria.map(c=>'<div class="head">'+c.no+'. '+esc(c.shortTitle)+'</div>').join('')+rows.map(p=>'<div><b>'+esc(p.name)+'</b><div class="mini">'+p.count+' รายการ | avg '+fmt(p.avg)+' ('+pctTxt(p.avg)+')</div></div>'+criteria.map(c=>heatCell(p.criteria[c.key])).join('')).join('')+'</div><div class="dash grid" style="margin-top:16px"><div class="panel"><div class="panel-head"><div><div class="panel-title">อันดับคะแนนรายเจ้าหน้าที่</div><div class="mini">เลือกเจ้าหน้าที่เพื่อดูรายละเอียด</div></div></div><div class="rank">'+(rows.map((p,i)=>'<button class="btn" style="justify-content:space-between" onclick="selectedStaff=\''+js(p.name)+'\';renderProfile()"><span>'+(i+1)+'. '+esc(p.name)+'</span><b>'+fmt(p.avg)+' · '+pctTxt(p.avg)+'</b></button>').join('')||'<div class="empty">ไม่มีข้อมูล</div>')+'</div></div><div class="panel" id="profilePanel"><div class="empty">เลือกเจ้าหน้าที่เพื่อดูรายละเอียด</div></div></div>';if(selectedStaff)renderProfile();}
...
function renderHelp(){
  ...
  h+=sec('คู่มือการใช้งานระบบ OSO Evaluation by IMM',
    '<span class="tag neutral">คู่มือเวอร์ชัน '+APP_VERSION+'</span> (ตรงกับเวอร์ชันระบบเสมอ)<br><br>ระบบประเมินเจ้าหน้าที่ Onsite Support สำหรับสำนักงานตรวจคนเข้าเมือง<br>บัญชีที่คุณใช้อยู่มีสิทธิ์: <span class="tag neutral">'+esc(roleNow)+'</span><br><br>ระบบแบ่งเป็น 2 ส่วน:'+ul([
  ...
  h+=sec('A. สำหรับผู้ประเมิน (เจ้าหน้าที่ ตม. — หน้าสาธารณะ ไม่ต้องล็อกอิน)',
    ol([
      ...
      'เลือก <b>ผลัด/ชื่อผู้ประเมิน</b> และ <b>เจ้าหน้าที่ Onsite Support</b> ที่จะประเมิน — เลือกจากรายชื่อในระบบเท่านั้น (ถ้าไม่มีชื่อ ให้แจ้งผู้ดูแลเพิ่มในเมนูจัดการรายชื่อ)',
  ...
  h+=sec('B. สำหรับ Senior / Manager (หลังล็อกอิน)',
    'เมนูด้านซ้ายและหน้าที่ใช้งานได้:'+ul([
      '<b>1. แดชบอร์ด</b> — สรุปภาพรวม: คะแนนเฉลี่ย (แสดงเป็น<b>เปอร์เซ็นต์</b>ด้วย), เจ้าหน้าที่ที่ถูกประเมิน, หัวข้อที่ควรติดตาม, ผลงานเด่น, รายการที่ควรติดตาม, จำนวนตามผลัด',
      '<b>2. รายการประเมิน</b> — ดูทุกรายการ ค้นหาได้ · กด <b>แก้ไข</b>/<b>x</b> · ปุ่ม <b>🧹 ลบรายการประเมินที่ซ้ำ</b> (ลบที่ตรงกันทุกช่อง + แถวตัวอย่าง)',
      '<b>3. สรุปรายเจ้าหน้าที่</b> — ตารางความร้อน + อันดับ · คลิกชื่อดูโปรไฟล์ + radar + แนวโน้ม · ปุ่ม <b>PDF</b> รายคน',
      '<b>4. วิเคราะห์ภาพรวม</b> — สรุปเชิงวิเคราะห์ทีม + คำสำคัญในข้อเสนอแนะ',
      '<b>จัดการรายชื่อ</b> — เพิ่ม/ลบ ผลัดและเจ้าหน้าที่ · ปุ่ม <b>ล้างชื่อซ้ำ</b> (รวมชื่อซ้ำ + ลบรายการตัวอย่าง) · <b>นำเข้า CSV</b> (กันซ้ำ + ข้ามแถวตัวอย่างอัตโนมัติ)',
  ...
  h+=sec('C. สำหรับ Admin (เพิ่มเติมจาก Senior/Manager)',
    'admin ทำได้ทุกอย่างของ senior/manager และมีเมนูพิเศษ <b>จัดการผู้ใช้ระบบ</b>:'+ul([
      ...
      '<b>อนุมัติคำขอลงทะเบียน</b> — ในหน้าจัดการผู้ใช้ มีแผง “คำขอลงทะเบียน (รออนุมัติ)” → เลือกสิทธิ์ → กด <b>อนุมัติ</b> (ผู้ใช้ได้รับอีเมลแจ้ง) หรือ <b>ปฏิเสธ</b>',
      '<b>ตั้งค่าส่งอีเมลอัตโนมัติ</b> — กำหนด เปิด/ปิด + วันที่ (1–31) + เวลา (ไทย) + อีเมลผู้รับ → ระบบส่งรายงาน DOCX (พร้อมกราฟ) ของเดือนก่อนหน้าให้อัตโนมัติ 1 ครั้ง/เดือน · ปุ่ม <b>✉ ทดสอบส่งทันที</b> กดได้ไม่จำกัด ไม่นับเป็นฉบับรายเดือน'
    ])+
    '<br><b>ข้อกำหนดรหัสผ่าน:</b> อย่างน้อย 6 ตัวอักษร ต้องผสมตัวอักษร+ตัวเลข เลี่ยงตัวเลขล้วน เช่น Onsite@2026');
  h+=sec('D. รายงานและการส่งออก',
    ul([
      ...
      '<b>ส่งรายงานทางอีเมล (DOCX)</b> — ในหน้าต่างรายงาน กรอกอีเมลผู้รับ (หลายคนคั่นด้วย ,) → กด “ดูตัวอย่าง” ตรวจก่อน → “ส่งอีเมล” แนบไฟล์ <b>DOCX</b> (ต้องตั้งค่า Brevo ก่อน — ดู SEND-EMAIL.md)',
      '<b>ส่งอีเมลอัตโนมัติทุกเดือน</b> — ตั้งค่าได้ที่เมนู “ตั้งค่าส่งอีเมลอัตโนมัติ” (admin) ดูวิธีติดตั้งใน AUTO-REPORT.md'
    ]));
  h+=sec('E. การเข้าระบบและรหัสผ่าน',
    ul([
      '<b>เข้าสู่ระบบ</b> — กด “เข้าสู่ระบบ” ที่หน้าแบบประเมิน → ใส่ email + รหัสผ่าน (มีปุ่ม 👁 ดู/ซ่อนรหัส)',
      '<b>ลงทะเบียนขอใช้งาน</b> — หน้า login กด “ลงทะเบียนขอใช้งานระบบ” → กรอกข้อมูล → ส่งคำขอ → <b>รอ admin อนุมัติ</b>จึงเข้าใช้งานได้ (เฉพาะ Senior/Manager)',
  ...
  h+=sec('F. ปัญหาที่พบบ่อย',
    ul([
      ...
      '<b>ล็อกอินไม่ได้</b> — ดูข้อความสีแดง: รหัสไม่ถูกต้อง / <b>บัญชีรออนุมัติ</b> (แจ้ง admin อนุมัติ) / เชื่อมต่อไม่ได้ (เช็กเน็ต-ปิด VPN)',
...
async function buildReportDocxBlob(startDate,endDate,periodWord,periodLabel){
  ...
  body+=dKpiCards([['จำนวนประเมิน',String(s.total),'รายการ'],['เจ้าหน้าที่',String(s.evaluated),'คน'],['คะแนนเฉลี่ย',s.avgScore.toFixed(2),pctTxt(s.avgScore)+' · '+s.band],['หัวข้อโฟกัส',weak?weak.shortTitle:'-',weak?((s.criteriaAvg[weak.key]||0).toFixed(2)+' · '+pctTxt(s.criteriaAvg[weak.key]||0)):'-']]);
  body+=dHeading('กราฟคะแนนเฉลี่ยรายหัวข้อ');
  body+=dImage();
  body+=dHeading('คะแนนเฉลี่ยรายหัวข้อ');
  body+=dTable([['ลำดับ','หัวข้อประเมิน','คะแนนเฉลี่ย','ร้อยละ','ระดับ']].concat(criteria.map(c=>{const a=s.criteriaAvg[c.key]||0;return [String(c.no),c.shortTitle,a.toFixed(2),pctTxt(a),scoreBand(a)];})),[800,3800,1400,1400,1600]);
  body+=dHeading('ประเด็นที่ควรติดตาม');
  const fr=(s.risks||[]).map((p,i)=>[String(i+1),p.name,p.avg.toFixed(2),pctTxt(p.avg),p.band]);
  body+=fr.length?dTable([['ลำดับ','ชื่อเจ้าหน้าที่','คะแนนเฉลี่ย','ร้อยละ','ระดับ']].concat(fr),[800,3700,1400,1400,1700]):dPar('ไม่พบเจ้าหน้าที่ที่มีคะแนนเฉลี่ยต่ำกว่าเกณฑ์ติดตามในรอบรายงานนี้',{color:'6a7d9b'});
  ...
  const pr=people.filter(p=>p.count).map((p,i)=>[String(i+1),p.name,String(p.count),p.avg.toFixed(2),pctTxt(p.avg),p.band]);
...
function buildReportInner(start,end,word,label){
  ...
  h+='<h2>คะแนนเฉลี่ยรายหัวข้อ</h2>'+tbl(['ลำดับ','หัวข้อประเมิน','คะแนนเฉลี่ย','ร้อยละ','ระดับ'],criteria.map(c=>{const a=s.criteriaAvg[c.key]||0;return [c.no,esc(c.shortTitle),fmt(a),pctTxt(a),esc(scoreBand(a))];}),['8%','40%','17%','16%','19%']);
  h+='<h2>ประเด็นที่ควรติดตาม</h2>'+tbl(['ลำดับ','ชื่อเจ้าหน้าที่','คะแนนเฉลี่ย','ร้อยละ','ระดับ'],(s.risks||[]).map((p,i)=>[i+1,esc(p.name),fmt(p.avg),pctTxt(p.avg),esc(p.band)]),['8%','40%','17%','16%','19%']);
...
async function emailReport(to,start,end,word,label,suffix){
  ...
  const msg='เรียน ผู้เกี่ยวข้อง\n\n'+
    ...
    '• คะแนนเฉลี่ยรวม: '+fmt(s.avgScore)+' / 5 (คิดเป็น '+pctTxt(s.avgScore)+' · ระดับ '+s.band+')\n'+
...
async function renderAutoReport(){
  ...
  $('content').innerHTML=
    ...
    '<div class="mini" style="margin:6px 0 14px;line-height:1.7">ระบบจะส่งรายงาน <b>DOCX พร้อมกราฟ</b> ทางอีเมลอัตโนมัติ ตาม <b>วันและเวลา</b> ที่กำหนดด้านล่าง (เวลาไทย) โดยใช้ข้อมูล <b>วันที่ 1 ถึงสิ้นเดือนของเดือนก่อนหน้า</b></div>'+
...
async function downloadPdf(name){
  ...
  const rowsHtml=criteria.map(c=>'<tr><td>'+esc(c.shortTitle)+'</td><td style="text-align:center">'+fmt(p.criteria[c.key])+'</td><td style="text-align:center">'+pctTxt(p.criteria[c.key])+'</td><td style="text-align:center">'+esc(scoreBand(p.criteria[c.key]))+'</td></tr>').join('');
  ...
  const html='<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><title>รายงาน '+esc(name)+'</title>'+
    ...
    '<div class="kpi"><div class="card"><b>'+esc(name)+'</b><span>เจ้าหน้าที่ Onsite Support</span></div><div class="card"><b>'+fmt(p.avg)+' / 5</b><span>คะแนนเฉลี่ย ('+pctTxt(p.avg)+' · '+esc(p.band)+')</span></div><div class="card"><b>'+p.count+'</b><span>จำนวนครั้งที่ประเมิน</span></div></div>'+
    '<h2>คะแนนเฉลี่ยรายหัวข้อ</h2><table><thead><tr><th>หัวข้อ</th><th style="text-align:center">คะแนนเฉลี่ย</th><th style="text-align:center">ร้อยละ</th><th style="text-align:center">ระดับ</th></tr></thead><tbody>'+rowsHtml+'</tbody></table>'+
